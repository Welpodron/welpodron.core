((window) => {
  ...
  class Zone {
    ...
    constructor({ element, sessid, config = {} }: ZonePropsType) {
      this.sessid = sessid;
    ...
    handleDocumentClick = async (event: MouseEvent) => {
      ...
      const action = (target as Element).getAttribute(ATTRIBUTE_ACTION);
      ...
      const controls = document.querySelectorAll(
        `[${ATTRIBUTE_BASE_ID}="${this.element?.getAttribute(
          ATTRIBUTE_BASE_ID
        )}"][${ATTRIBUTE_ACTION}][${ATTRIBUTE_CONTROL}]`
      );
